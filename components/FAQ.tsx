      ...
      <div
        ...
        <div
          ...
          <Image
            ...
            height={450}
            alt="FAQs"
            className="w-full h-auto object-cover rounded-tr-[100px] rounded-bl-[100px] border border-gray-200" />
        </div>
        <div
            ...
            return (
              <div key={idx}>
                <button
                  ...
                  <span
                    className={`flex items-center text-start text-lg font-semibold ${isOpen ? 'text-[#E22128]' : 'text-gray-800'
                      }`}>
                    {isOpen ? (
                ...
                <div
                  className={`overflow-hidden bg-white text-gray-700 transition-all duration-300 ease-in-out px-6 ${isOpen ? 'max-h-60 py-4' : 'max-h-0 py-0'}`}>
                  <div className="text-base">{item.answer}</div>
                </div>
                {idx < faqItems.length - 1 && (
                  <div className="border-t border-[#0f446ca1]" />
                )}
              </div>
            );
          })}
